...
import { X, FileText, HelpCircle, Search, Clock, Database, BookOpen } from "lucide-react";
      ...
      >
        ...
        <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-4">
            ...
            >
              ...
              {activity.type === "guide" && <BookOpen className="w-5 h-5" />}
            </div>
            <div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activity.type === "guide" && (
            <>
              {/* Knowledge Points */}
              {activity.content?.knowledge_points?.length > 0 && (
                <div className="space-y-2">
                  <h3 className="font-bold text-slate-900 dark:text-slate-100">
                    {t("Knowledge Points")} ({activity.content.knowledge_points.length})
                  </h3>
                  <div className="space-y-2">
                    {activity.content.knowledge_points.map((kp: any, idx: number) => (
                      <div
                        key={idx}
                        className="p-3 bg-slate-50 dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700"
                      >
                        <div className="flex items-center gap-2 mb-1">
                          <span className="w-5 h-5 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 text-xs font-bold flex items-center justify-center shrink-0">
                            {idx + 1}
                          </span>
                          ...
                          {kp.user_difficulty && (
                            <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400">
                              {kp.user_difficulty}
                            </span>
                          )}
                        </div>
                        {kp.knowledge_summary && (
              ...
              {activity.content?.summary && (
                <div className="space-y-2">
                  ...
                  <div className="p-6 bg-white dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm max-h-80 overflow-y-auto">
                    <div className="prose prose-slate dark:prose-invert max-w-none prose-sm">
                      <ReactMarkdown
                        remarkPlugins={[remarkGfm, remarkMath]}
                        rehypePlugins={[rehypeKatex]}
                      >
                        {processLatexContent(activity.content.summary)}
                      </ReactMarkdown>
                    </div>
                  </div>
                </div>
