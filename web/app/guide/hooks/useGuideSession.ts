...
const GUIDE_HISTORY_KEY = "guide_learning_history";
...
export function useGuideSession() {
  ...
  useEffect(() => {
    ...
    const persistedHistory = loadFromStorage<LearningHistory[]>(GUIDE_HISTORY_KEY, []);
    ...
    if (persistedHistory.length > 0) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- hydration restore
      setLearningHistory(persistedHistory);
    }
    ...
    try {
      ...
      if (data.success) {
        if (data.status === "completed") {
          // Build and persist history entry first
          const stored = loadFromStorage<LearningHistory[]>(GUIDE_HISTORY_KEY, []);
          setSessionState((prev) => {
            const newEntry: LearningHistory = {
              id: `history-${Date.now()}`,
              notebook_name: prev.notebook_name,
              knowledge_points: prev.knowledge_points,
              summary: data.summary || "",
              completed_at: Date.now(),
            };
            const updated = [newEntry, ...stored].slice(0, 20);
            saveToStorage(GUIDE_HISTORY_KEY, updated);
