...
class ReportingAgent(BaseAgent):
    ...
    async def _write_section_body(
        ...
        try:
            ...
            # Fallback: LLM returned plain Markdown instead of JSON — use raw response directly
            stripped = resp.strip() if resp else ""
            if stripped:
                self.logger.warning(
                    f"write_section_body: JSON parse failed, using raw LLM response ({len(stripped)} chars)"
                )
                return stripped
            raise ValueError("Unable to parse LLM returned section content and raw response is empty.")
    ...
    async def _write_section_with_subsections(
        ...
        try:
            ...
            if stripped:
                self.logger.warning(
                    f"write_section_with_subsections: JSON parse failed, using raw LLM response ({len(stripped)} chars)"
                )
                return stripped
            raise ValueError("Unable to parse LLM returned section content and raw response is empty.")
