...
async def edit_text(request: EditRequest):
    """Co-writer text editing — powered by LangGraph 1.0.9."""
    return await edit_text_lg(request)


async def _edit_text_legacy(request: EditRequest):
...
async def narrate_content(request: NarrateRequest):
    """Co-writer narration — powered by LangGraph 1.0.9."""
    return await narrate_content_lg(request)


async def _narrate_content_legacy(request: NarrateRequest):
    """Original implementation preserved for reference."""
    try:
...
# =============================================================================
# LangGraph REST Endpoints (parallel testing — replaces /edit and /narrate after validation)
# =============================================================================


@router.post("/edit/lg", response_model=EditResponse)
async def edit_text_lg(request: EditRequest):
    """
    LangGraph-based Edit endpoint.

    Drop-in replacement for /edit using LangGraph 1.0.9 + LangChain 1.2.x.
    Graph: START → retrieve_context_node → edit_node → END
    """
    ...
    from fastapi import Request  # not used — ainvoke directly
    ...
    try:
        ...
        initial_state = {
            ...
            # narrate fields (not used by edit graph)
            "content": "",
            "style": "friendly",
            "voice": "",
            "skip_audio": True,
...
@router.post("/narrate/lg")
async def narrate_content_lg(request: NarrateRequest):
    """
    LangGraph-based Narrate endpoint.

    Drop-in replacement for /narrate using LangGraph 1.0.9 + LangChain 1.2.x.
    Graph: START → narrate_node → END
    """
    import uuid as _uuid
    ...
    try:
        ...
        initial_state = {
            ...
            "skip_audio": request.skip_audio,
            "language": language,
            # edit fields (not used by narrate graph)
            "text": "",
            "instruction": "",
            "action": "rewrite",
            "source": "none",
