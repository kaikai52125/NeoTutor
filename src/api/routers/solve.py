...
async def websocket_solve(websocket: WebSocket):
    """Solve WebSocket — powered by LangGraph 1.0.9."""
    await websocket_solve_lg(websocket)


async def _websocket_solve_legacy(websocket: WebSocket):
...
# =============================================================================
# LangGraph WebSocket Endpoint (parallel testing — replaces /solve after validation)
# =============================================================================


@router.websocket("/solve/lg")
async def websocket_solve_lg(websocket: WebSocket):
    """
    LangGraph-based Solve endpoint.

    Drop-in replacement for /solve using LangGraph 1.0.9 + LangChain 1.2.x.
    Implements the dual-loop StateGraph:
      Analysis Loop: investigate → note → (loop) → plan
      Solve Loop:    plan → exec_tools → solve_step → response → advance_step → finalize

    Message format (same as /solve):
    {
    ...
    try:
        ...
        # Output dir mirrors original MainSolver behaviour
        from pathlib import Path as _Path
        output_base = _Path("data/user/solve") / run_id
        output_base.mkdir(parents=True, exist_ok=True)
        output_dir = str(output_base)

        await safe_send({"type": "session_id", "session_id": session_id})
        await safe_send({"type": "status", "stage": "started", "message": "starting"})

        logger.info(
            f"LangGraph Solve request: question={question[:60]!r}, run_id={run_id}"
        )
        ...
        try:
            history_manager.add_entry(
                ...
                summary=final_answer[:100] + "..." if final_answer else "",
            )
        except Exception as hist_exc:
            logger.warning(f"Failed to save solve history: {hist_exc}")

        logger.info(
            f"LangGraph Solve completed: run_id={run_id}, answer={len(final_answer)} chars"
        )

    except WebSocketDisconnect:
        logger.debug("Client disconnected from LangGraph solve")
    except Exception as exc:
        logger.error(f"LangGraph Solve error: {exc}", exc_info=True)
        try:
